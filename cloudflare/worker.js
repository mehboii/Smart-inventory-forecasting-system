...
async function api(request, env, path) {
  ...
  if (path === '/auth/team' && method === 'GET') { const members = (await env.DB.prepare('SELECT id,name,role,created_at FROM users ORDER BY created_at').all()).results; const owner = await env.DB.prepare("SELECT id,name,email FROM users WHERE role='admin' ORDER BY created_at LIMIT 1").first(); return json({ owner: owner || null, memberCount: members.length, members }); }
  if (path.startsWith('/auth/admin/') && user.role !== 'admin') return fail('Admin role required', 403);
  if (path === '/auth/admin/users' && method === 'GET') { const users = (await env.DB.prepare('SELECT id,name,email,role,created_at FROM users ORDER BY created_at').all()).results; const owner = await env.DB.prepare("SELECT id FROM users WHERE role='admin' ORDER BY created_at LIMIT 1").first(); return json({ users, ownerId: owner?.id || null }); }
  const userRoleMatch = path.match(/^\/auth\/admin\/users\/(\d+)\/role$/);
  if (userRoleMatch && method === 'PATCH') { const role = String(body.role || ''); const id = Number(userRoleMatch[1]); if (!validRole(role)) return fail('Role must be admin, manager, or user'); const target = await env.DB.prepare('SELECT * FROM users WHERE id=?').bind(id).first(); if (!target) return fail('User not found', 404); const owner = await env.DB.prepare("SELECT id FROM users WHERE role='admin' ORDER BY created_at LIMIT 1").first(); if (target.id === owner?.id) return fail('The main owner role cannot be changed', 403); const admins = await env.DB.prepare("SELECT COUNT(*) AS count FROM users WHERE role='admin'").first(); if (target.role === 'admin' && role !== 'admin' && admins.count === 1) return fail('At least one administrator must remain'); await env.DB.prepare('UPDATE users SET role=? WHERE id=?').bind(role,id).run(); return json({ user: publicUser(await env.DB.prepare('SELECT * FROM users WHERE id=?').bind(id).first()) }); }
  const deleteUserMatch = path.match(/^\/auth\/admin\/users\/(\d+)$/);
  if (deleteUserMatch && method === 'DELETE') { const id = Number(deleteUserMatch[1]); const target = await env.DB.prepare('SELECT * FROM users WHERE id=?').bind(id).first(); if (!target) return fail('User not found', 404); const owner = await env.DB.prepare("SELECT id FROM users WHERE role='admin' ORDER BY created_at LIMIT 1").first(); if (target.id === owner?.id) return fail('The main owner cannot be removed',403); if (target.role === 'admin' && user.id !== owner?.id) return fail('Only the main owner can remove another administrator',403); await env.DB.prepare('DELETE FROM users WHERE id=?').bind(id).run(); return json({ message: 'User removed' }); }
  ...
  const invitationMatch = path.match(/^\/auth\/admin\/invitations\/(\d+)$/);
  if (invitationMatch && method === 'DELETE') { const result = await env.DB.prepare('DELETE FROM invitations WHERE id=? AND accepted_at IS NULL').bind(Number(invitationMatch[1])).run(); return result.meta.changes ? json({ message: 'Invitation cancelled' }) : fail('Pending invitation not found',404); }
